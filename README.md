<!-- ===================== HEADER ===================== -->

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:93C5FD,50:3B82F6,100:2563EB&height=130&section=header" alt="" />

<a href="https://instadrop.web.app">
  <img src="public/logo.png" height="72" alt="Instadrop" />
  &nbsp;
  <img src="public/brand.png" height="72" alt="Instadrop" />
</a>

<br/><br/>

<a href="https://instadrop.web.app">
  <img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=22&pause=900&color=2563EB&center=true&vCenter=true&width=640&height=45&lines=Download+Instagram+media+in+one+click;Reels+%C2%B7+Posts+%C2%B7+Carousels+%C2%B7+Highlights;Audio+%E2%86%92+MP3%2C+right+in+your+browser;No+ads+%C2%B7+No+trackers+%C2%B7+No+login" alt="Instadrop" />
</a>

<br/>

<a href="https://instadrop.web.app">
  <img src="https://img.shields.io/badge/Live-instadrop.web.app-2563EB?style=for-the-badge&logo=firefoxbrowser&logoColor=white" alt="Live demo" />
</a>
<img src="https://img.shields.io/badge/PWA-Installable-5A0FC8?style=for-the-badge&logo=pwa&logoColor=white" alt="PWA" />
<img src="https://img.shields.io/badge/Vanilla_JS-No_frameworks-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" alt="Vanilla JS" />
<img src="https://img.shields.io/badge/Firebase-Hosting-FFCA28?style=for-the-badge&logo=firebase&logoColor=black" alt="Firebase Hosting" />
<img src="https://img.shields.io/badge/FFmpeg.wasm-Audio_%E2%86%92_MP3-007808?style=for-the-badge&logo=ffmpeg&logoColor=white" alt="FFmpeg.wasm" />
<img src="https://img.shields.io/badge/License-MIT-22C55E?style=for-the-badge&logo=opensourceinitiative&logoColor=white" alt="MIT" />

<br/><br/>

<a href="https://instadrop.web.app">
  <img src="https://img.shields.io/badge/▶️_Open_instadrop-2563EB?style=for-the-badge&logo=firefoxbrowser&logoColor=white" alt="Open Instadrop" />
</a>

</div>

---

<!-- ===================== ABOUT ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Travel%20and%20places/Rocket.png" width="30" valign="middle" alt="" /> About

**Instadrop** is a privacy-friendly, browser-based Instagram downloader. Paste a link
and grab **reels, posts, carousels, profile pictures and story highlights** at the
original quality — plus extract any reel's soundtrack as **MP3**.

Everything runs in your browser: the media is resolved from Instagram's own public
endpoints and handed straight to you. **No ads, no login, no trackers.**

<div align="center">

| 🌐 Live app | 🧩 Install as app | 📱 Works on |
| :---: | :---: | :---: |
| [instadrop.web.app](https://instadrop.web.app) | Add to Home Screen (PWA) | Android · iOS · Desktop |

</div>

---

<!-- ===================== FEATURES ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Activities/Sparkles.png" width="30" valign="middle" alt="" /> Features

<table>
<tr>
<td width="50%" valign="top">

**<img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Video%20Camera.png" width="20" valign="middle" alt="" /> Media you can grab**

- 🎬 **Reels & posts** — single videos and photos at original quality
- 🖼️ **Carousels** — download every slide of a multi-image post in one go
- 👤 **Profile pictures** — original size, no login needed
- 🌟 **Story highlights** — public highlights with a couple of taps
- 🎵 **Audio → MP3** — extract any reel's soundtrack, converted in the browser

</td>
<td width="50%" valign="top">

**<img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Gear.png" width="20" valign="middle" alt="" /> Built-in comfort**

- 🚫 **No ads, no trackers, no popups, no redirects** — just a download button
- 🔒 **Private by design** — your links and files never leave your device
- 📱 **Installable PWA** — works offline once installed
- 🔗 **App shortcuts** — long-press the icon: reel, profile pic, or MP3
- 🌓 **Light / Dark / System** themes
- ⚡ **Fast & dependency-light** — vanilla JavaScript, no framework

</td>
</tr>
</table>

---

<!-- ===================== HOW TO USE ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Mobile%20Phone.png" width="30" valign="middle" alt="" /> How to use

1. Copy the URL of the **reel, post, carousel, profile or public highlight** from Instagram.
2. Paste it into the box (or tap **Paste** to grab it from your clipboard).
3. Press **Download** — the media is resolved in seconds.
4. Preview the result and hit **Save** — or grab the **MP3** of the audio.

> ℹ️ Works with `instagram.com/reel/…`, `instagram.com/name/p/…` and
> profile / highlight links. Profile pictures arrive at the size Instagram serves
> to anonymous visitors.

---

<!-- ===================== TECH ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Hammer%20and%20Wrench.png" width="30" valign="middle" alt="" /> Tech stack

| Layer | What it uses |
| :--- | :--- |
| **Frontend** | Vanilla JavaScript + CSS (no frameworks) |
| **UI** | [RippleUI](https://rippleui.com) component styles + Tailwind preflight |
| **Audio** | [FFmpeg.wasm](https://ffmpegwasm.netlify.app/) for in-browser MP3 conversion |
| **Hosting** | Firebase Hosting |
| **Offline / install** | PWA — `manifest.webmanifest` + a service worker cache |

---

<!-- ===================== STRUCTURE ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Objects/Books.png" width="30" valign="middle" alt="" /> Project structure

```text
├── firebase.json             hosting config (clean URLs, caching, headers)
├── .firebaserc               Firebase project id
├── public/
│   ├── index.html            page + SEO / OG / JSON-LD metadata
│   ├── styles.css            all styling
│   ├── script.js             all logic
│   ├── manifest.webmanifest  PWA manifest (installable app)
│   ├── sw.js                 service worker (offline cache)
│   ├── logo.png / brand.png / favicon.ico   branding
│   ├── icon-192 / icon-512.png              PWA icons
│   ├── og-image.png          social share card
│   ├── robots.txt / sitemap.xml             SEO
│   └── 404.html              custom 404
└── functions/                Telegram bot backend (optional, see below)
```

---

<!-- ===================== DEPLOY ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Travel%20and%20places/Rocket.png" width="30" valign="middle" alt="" /> Deploy

**Option A — GitHub (no terminal)**

1. Push this repo to GitHub.
2. [console.firebase.google.com](https://console.firebase.google.com) → **Add project** (`instadrop`).
3. **Build → Hosting → Get started** → connect your GitHub repo → Deploy.
4. Your site is live at `https://instadrop.web.app`, auto-redeploying on every push.

**Option B — Firebase CLI**

```sh
npm i -g firebase-tools
firebase login
firebase use --add   # pick the instadrop project
firebase deploy
```

---

<!-- ===================== BOT ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Smilies/Robot.png" width="30" valign="middle" alt="" /> Telegram bot

Alongside the website there is an optional Telegram bot that sends reels/posts straight
into a chat as video files. It reuses the **same** media-download endpoints the website
uses — no second downloader — and lives in its own `functions/` folder, so the website
is untouched.

- Setup guide: see **[BOT_SETUP.md](BOT_SETUP.md)**
- Backend: `functions/` (Firebase Cloud Functions)
- Secrets: `TELEGRAM_BOT_TOKEN` + `WEBHOOK_SECRET` via `firebase functions:secrets:set`
- Webhook: `node scripts/setup-webhook.js`

---

<!-- ===================== NOTES ===================== -->

## <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Symbols/Warning.png" width="30" valign="middle" alt="" /> Notes

- **Downloading content:** only download media you have the right to — your own posts,
  or content the owner has explicitly shared for downloading. Respect creators' rights
  and Instagram's terms of service.
- Instadrop is **not affiliated with, endorsed by, or sponsored by** Meta / Instagram.
- The public Instagram endpoints are CORS-restricted in browsers; if downloads fail in
  production, a small serverless proxy (Cloudflare Worker or Firebase Function) can be
  dropped in — the code is structured so this is a one-file swap.

---

<!-- ===================== FOOTER ===================== -->

<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=18&pause=900&color=2563EB&center=true&vCenter=true&width=420&height=40&lines=Ready+to+try+it%3F" alt="" />

<a href="https://instadrop.web.app">
  <img src="public/logo.png" height="56" alt="Instadrop" />
</a>

<br/><br/>

<sub>© Instadrop · MIT License · Made with <img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Smilies/Red%20Heart.png" width="16" valign="middle" alt="love" /></sub>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:2563EB,50:3B82F6,100:93C5FD&height=120&section=footer" alt="" />

</div>
